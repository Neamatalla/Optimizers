...
When the Obsidian MCP server is available, always treat the Obsidian vault folder `Agents Brain` as the shared working brain.

Before starting any new task:
- Search `Agents Brain` for relevant notes, decisions, preferences, project context, and prior work.
- Read the most relevant notes before making assumptions or beginning implementation.
- Use `Agents Brain` to preserve continuity across sessions.

After completing a task that creates durable context:
- Update the relevant notes in `Agents Brain`.
- Record important decisions, outcomes, project context, and reusable knowledge.
- Put unsorted but potentially useful information in `Agents Brain/04 - Inbox.md`.

Do not store secrets, credentials, private tokens, or sensitive personal data in Obsidian.
