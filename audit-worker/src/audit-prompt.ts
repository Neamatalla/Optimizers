...
import { summarizeSitePageSpeed, type PageSpeedReport } from "./pagespeed.js";
...
export interface AuditPromptInput {
  ...
  // Every page the deterministic sweep (discover-pages.ts + pagespeed.ts's
  // fetchSitePageSpeed) actually checked, homepage included/first — not
  // just the one URL the visitor typed in. `pagespeed` above is always
  // sitePageSpeed[0]'s report (the homepage), kept as its own field since
  // most of the checklist's PageSpeed-tied points were written assuming a
  // single-page report; sitePageSpeed is the fuller picture on top of that.
...
/**
 * "Website" isn't a single fixed list any more — it's the full 46-item
 * WEBSITE_CHECKLIST on the "visitor has neither GA4 nor GTM" route, but the
 * 21-item curated WEBSITE_CODE_CHECKLIST on the "visitor has exactly one of
 * GA4/GTM" route (see scoring.ts's countedCategories and checklist.ts's own
 * doc comment for why). GA4/GTM/PageSpeed are each always the one fixed
...
function buildPrompt(input: AuditPromptInput): string {
  ...
  // PageSpeed now has its own real checklist (PAGESPEED_CHECKLIST, backed
  // by the deterministic sweep) — no longer filtered out here the way an
  // empty checklist previously needed to be.
...
PageSpeed Insights, homepage (mobile + desktop):
${JSON.stringify(input.pagespeed, null, 2)}

Site-wide PageSpeed sweep — every page actually checked this run (homepage
first, then up to ${input.sitePageSpeed.length - 1} more discovered from the
site's sitemap.xml or, absent one, its own homepage links). The summary below
is computed deterministically in code, not estimated — trust these numbers
exactly, don't re-derive or round them differently:
${JSON.stringify(summarizeSitePageSpeed(input.sitePageSpeed), null, 2)}

Per-page detail for the sweep, if you need to cite a specific page's numbers:
${JSON.stringify(input.sitePageSpeed, null, 2)}
...
JS-free static crawl of the page source, PageSpeed Insights, and — only when
noted in a section below — a real headless browser.
...
signal, PageSpeed data, general best-practice reasoning for a business like
...
real, written finding, the same way a broken one does:
1. It's clean → add one finding with "status": "pass". "issue" states what
   you actually checked and the real result/value you found (e.g. "Property
   timezone is Asia/Dubai, currency AED — matches the business's real UAE
   market" or "Realtime activeUsers returned 14, plausible for a site live
   right now") — never a generic restatement of the checklist point's own
   title, never "N/A", never "no issue found" with nothing backing it up.
   "businessImpact" says why that result is good / what it protects.
2. It's NOT in the expected state → add one finding with "status": "fail",
   same "issue"/"businessImpact" fields as before (what's wrong, referencing
   real evidence; why it costs the business).
...
The visitor only has ${input.tools[0]}, not both GA4 and GTM. The other tool is NOT evaluated this run at all — don't try to check it, don't guess about it from the crawl signal, it's simply not part of this audit. Instead, its 25 points are replaced by a real, fully-evaluable website-code audit: the ${WEBSITE_CODE_CHECKLIST.length}-item "Website" checklist below (a curated slice, not the full Website checklist) plus the ${PAGESPEED_CHECKLIST.length}-item PageSpeed checklist — ${input.tools[0]} (25) + Website (${WEBSITE_CODE_CHECKLIST.length}) + PageSpeed (${PAGESPEED_CHECKLIST.length}) = 50 total. This is deliberate: a real check of the site's own code (image weight, dataLayer/ecommerce implementation, duplicate tag/event firing, consent mode, network anomalies, console errors) is more useful to the business than a weak "probably not configured" guess about a tracking tool they don't even use.
...
steps below reference ids that only exist in the FULL 46-item checklist and
won't be present when this run is on the 21-item curated slice instead; skip
...
7. If the site-wide sweep (per-page detail above) found a cart/checkout-looking URL, navigate there specifically and repeat the console-error check (WEB-54), and inspect the DOM for the primary Add-to-Cart/checkout CTA — present, visible, not disabled (WEB-56).
8. This browser is your only source of real runtime behavior this run — the static crawl signal above is HTML-only, and PageSpeed's own audits are lab data, not live interaction. Every finding based on what you directly observed in the browser gets dataSource "live"; anything inferred from the static crawl or PageSpeed data instead stays "detection".
9. If the browser tools error out or a page fails to load, don't fail the whole audit — note it as a low-severity WEB finding ("couldn't run a live browser check this time") and continue with the static/PageSpeed signal alone for whatever that leaves uncovered.
...
${input.ga4OAuthData
  ...
  : `No OAuth data for GA4 — fall back to the shared-service-account MCP tool (mcp__ga4-admin). That account is used by many different client businesses over time, so identification must be by EXACT ID MATCH ONLY, never by business name or any fuzzy signal. Candidate measurement IDs (crawled and/or visitor-typed, either source equally valid as a match key but never as proof of ownership on its own): ${JSON.stringify(ga4Candidates)}. List accessible properties, find each one's web data stream(s), and ONLY use a property whose stream's measurementId is an EXACT string match to one of those IDs. If the candidate list is empty (GA4 may load dynamically via GTM rather than a literal gtag.js snippet — inspect an already-matched GTM container's tags for a GA4 Configuration tag's measurementId as a secondary attempt, still exact-match only) or nothing matches, evaluate the GA4 checklist against the crawl signal + general best practice instead and mark dataSource "detection". An exact match is the only condition under which you may mark dataSource "live".`}
...
each) when the visitor has both, ${categories.length === 3 ? `that one tool (25) + Website (${WEBSITE_CODE_CHECKLIST.length}) + PageSpeed (${PAGESPEED_CHECKLIST.length}) when they have exactly one (see "Why a website-code audit runs alongside ${input.tools[0] ?? ""} this time" above)` : `Website (46) + PageSpeed (4) when they have neither`}.
  ...
  "categories": [
    {
      ...
      "findings": [
        { "checklistId": "<the EXACT id of the checklist point this finding is about, e.g. GA4-15 — must be copied verbatim from that category's checklist above, and every point gets exactly one of these, pass or fail>", "status": "<pass|fail>", "issue": "<real, specific: what you actually checked and what you found — for a pass, the real result that's fine; for a fail, what's wrong. Never generic, never a restatement of the checklist point's own title/expectedState>", "businessImpact": "<why it matters, in plain language — for a pass, why that result is good; for a fail, why it costs the business>", "dataSource": "<live|detection|public>", "severity": "<critical|medium|low — from the checklist point's own severity for a fixed checklist point (a finding on a critical-severity point is a critical finding), EXCEPT for a GA4-D1..GA4-D15 finding, where you set severity yourself based on that insight's real business impact — see \"GA4 deep-analysis slots\" above>" }
      ]
    }
  ]
}`;
 ...
 * 25, 25, 21, 4, or 46, whichever checklist this category's route uses —
...
export function computeCategoryResult(raw: RawCategoryResult, checklist: ChecklistPoint[]): CategoryResult {
  ...
  const findings: CategoryFinding[] = checklist.map(point => {
    ...
    return {
      checklistId: point.id,
      issue: "No specific finding was reported for this check this run.",
      businessImpact: "",
...
export async function runAudit(input: AuditPromptInput): Promise<CategoryResult[]> {
  ...
  const raw = await runClaudeHeadless({ prompt, mcpConfigPath: input.mcpConfigPath, allowedTools });
